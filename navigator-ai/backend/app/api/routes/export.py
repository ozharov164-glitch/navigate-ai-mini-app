...
from backend.app.core.security import create_export_token
...
@router.get("/token")
async def export_token(user: User = Depends(get_current_user)):
    """Краткоживущий токен для скачивания через window.open (?token=)."""
    return {"token": create_export_token(user.id), "expires_in": 300}


@router.get("/ical")
async def export_ical(user: User = Depends(get_export_user), db: AsyncSession = Depends(get_db)):
...
@router.get("/pdf")
async def export_pdf(user: User = Depends(get_export_user), db: AsyncSession = Depends(get_db)):
